                                ...
                                <div class="exzoom_img_box">
                                    ...
                                    <ul class='exzoom_img_ul'>
                                        ...
                                        @foreach ($product->productImageGalleries as $img)
                                            <li><img class="zoom ing-fluid w-100" src="{{ asset($img->image) }}"
                                                    alt="product"></li>
                                        @endforeach
                                    </ul>
                                </div>
                        ...
                        <div class="wsus__pro_details_text">
                            ...
                            @if ($product->quantity > 0)
                                <p class="wsus__stock_area"><span class="in_stock">in
                                        stock</span>({{ $product->quantity }} items)</p>
                            @elseif ($product->quantity == 0)
                                <p class="wsus__stock_area"><span class="in_stock">out of
                                        stock</span>({{ $product->quantity }} items)</p>
                            @endif
                            @if (checkDiscount($product))
                                <h4>{{ $product->offer_price }}EGP<del>{{ $product->price }}EGP</del></h4>
                            @else
                                <h4>{{ $product->price }}</h4>
                            @endif
                            <p class="review">
                                                    ...
                                                    <select class="select_2" name="variants_items[]">
                                                        @foreach ($variant->productVariantItem as $variantItem)
                                                            @if ($variantItem->status !== 0)
                                                                <option
                                                                    value="{{ $variantItem->id }}"{{ $variantItem->is_default == 1 ? 'selected' : '' }}>
