    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $deleteButton = "<a href='" . route('admin.product-image-gallery.destroy', $query->id) . "' class='btn btn-danger ml-2 delete-item'><i class='far fa-trash-alt'></i></a>";
                return $deleteButton;
            })
            ->addColumn('images', function ($query) {
                return $image = "<img width='200px' src='" . asset($query->image) . "'></img>";
            })
